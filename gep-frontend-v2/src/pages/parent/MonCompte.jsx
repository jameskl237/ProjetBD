...
export default function ParentMonCompte() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    Promise.allSettled([
      personnesApi.me(),
      elevesApi.list(),
    ]).then(([aboRes, elevesRes]) => {
      if (aboRes.status === 'fulfilled') setValues(aboRes.value)
      else setError(aboRes.reason?.response?.data?.error || 'Impossible de charger le profil')
      if (elevesRes.status === 'fulfilled') setEnfants(elevesRes.value || [])
    })
  }, [])
  ...
  if (!values) return (
    <div>
      <PageHeader title="Mon compte" subtitle="Gestion de votre profil parent" />
  ...
  const TABS = [
    ...
    { key: 'enfants', label: 'Mes enfants', icon: '👶' },
  ]
  ...
  return (
    <div>
      ...
      <Card style={{
        ...
        border: 'none', color: '#fff', boxShadow: '0 16px 40px rgba(91, 33, 182, 0.20)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, opacity: 0.9 }}>Parent — {user?.login || '—'}</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <span style={pillStyle}>👤 Parent</span>
            {values.email && <span style={pillStyle}>📧 {values.email}</span>}
            {enfants.length > 0 && <span style={pillStyle}>👶 {enfants.length} enfant{enfants.length > 1 ? 's' : ''}</span>}
          </div>
        </div>
      </Card>
        ...
        <form onSubmit={handleProfileSubmit}>
          <Card style={{ maxWidth: 640 }}>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Téléphone</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{values.phone || values.mobile || '—'}</div>
              </div>
            </div>
        ...
        <Card style={{ maxWidth: 640 }}>
          ...
          {enfants.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {enfants.map((enfant) => {
                const insc = enfant.inscriptions?.[0]
                const classe = insc?.classe?.libelle || null
                const tuteurs = enfant.tuteurs || []
                return (
                  <div key={enfant.matricule} style={{
                    display: 'flex', alignItems: 'center', gap: 14,
                    padding: '14px 16px', borderRadius: 10,
                    border: '1px solid var(--border)',
                    transition: 'box-shadow .15s ease',
                  }}
                    onMouseEnter={(ev) => { ev.currentTarget.style.boxShadow = '0 4px 12px rgba(15, 23, 42, 0.06)' }}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 2 }}>
                        ...
                        <span>🆔 {enfant.matriculeCode || `#${enfant.matricule}`}</span>
                        {enfant.sexe === 1 ? <span>♂ Masculin</span> : <span>♀ Féminin</span>}
                      </div>
                    </div>
                    <Badge tone="info">{classe || 'Non inscrit'}</Badge>
                  </div>
                )
              })}
            </div>
